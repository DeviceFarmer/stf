...
type AddDeviceResult = 'not found' | GroupDocument | null
...
function getDevice(req: ApiRequest, serial: string) {
...
function checkConflicts(id: string, devices: string[], dates: GroupDates[]) {
  function computeConflicts(
    conflicts: GroupConflict[]
  , liteGroup: Pick<GroupDocument, 'id' | 'devices' | 'dates'>
  , otherGroup: GroupDocument
  ) {
...
, stop: Date
  ...
  if (!Object.prototype.hasOwnProperty.call(apiutil.CLASS_DURATION, _class)) {
    return Promise.resolve(apiutil.respond(res, 400, 'Bad Request (Invalid class)'))
  }
  ...
  if (((stop as Instant) - (start as Instant)) > apiutil.CLASS_DURATION[_class]!) {
...
function addGroupDevices(req: ApiRequest<SerialsBody>, res: Response) {
  ...
  function addGroupDevice(group: GroupDocument, serial: string) {
    const lock: {device?: DeviceDocument, group?: {id: string}} = {}
    ...
    return dbapi.lockBookableDevice(req.user.groups.subscribed, serial).then(function(stats) {
      ...
      return dbapi.lockGroup(lock.device!.group.origin).then<AddDeviceResult>(function(stats) {
        if (!stats.replaced) {
          return apiutil.lightComputeStats(res, stats)
        }
  ...
  function _addGroupDevices(lockedGroup: GroupDocument, serials: string[]) {
    let results: unknown[] = []
    ...
    .then(function() {
      ...
      if (!results.length) {
        ...
        if (!results.length) {
          ...
          , {group: apiutil.publishGroup(results[results.length - 1] as GroupDocument)})
        }
      }
    })
    .catch(function(err) {
  ...
  lockutil.lockGroup(req, res, lock).then<Responded>(function(lockingSuccessed) {
    if (lockingSuccessed) {
...
function addGroupDevice(req: ApiRequest<SerialsBody>, res: Response) {
  apiutil.redirectApiWrapper('serial', addGroupDevices, req, res)
}

function removeGroupDevices(req: ApiRequest<SerialsBody>, res: Response) {
  const serials = apiutil.getBodyParameter(req.body, 'serials')
  const target = apiutil.getQueryParameter(req.swagger.params.redirected) ? 'device' : 'devices'
  const lock: ApiLock = {}
...
function removeGroupDevice(req: ApiRequest<SerialsBody>, res: Response) {
  apiutil.redirectApiWrapper('serial', removeGroupDevices, req, res)
}

function getGroupDevice(req: ApiRequest, res: Response) {
  const id = req.swagger.params.id.value
...
function getGroupUser(req: ApiRequest, res: Response) {
  const id = req.swagger.params.id.value
...
function getGroupUsers(req: ApiRequest, res: Response) {
  const id = req.swagger.params.id.value
...
function removeGroupUsers(req: ApiRequest<EmailsBody>, res: Response) {
  const id = req.swagger.params.id.value
  ...
  function removeGroupUser(email: string, group: GroupDocument, rootGroup: GroupDocument) {
...
function removeGroupUser(req: ApiRequest<EmailsBody>, res: Response) {
  apiutil.redirectApiWrapper('email', removeGroupUsers, req, res)
}

function addGroupUsers(req: ApiRequest<EmailsBody>, res: Response) {
  const id = req.swagger.params.id.value
  const emails = apiutil.getBodyParameter(req.body, 'emails')
  const target = apiutil.getQueryParameter(req.swagger.params.redirected) ? 'user' : 'users'
  const lock: ApiLock = {}

  function addGroupUser(email: string) {
    const lock: ApiLock = {}
  ...
  function _addGroupUsers(emails: string[]) {
    let results: unknown[] = []
...
function addGroupUser(req: ApiRequest<EmailsBody>, res: Response) {
  apiutil.redirectApiWrapper('email', addGroupUsers, req, res)
}

function getGroup(req: ApiRequest, res: Response) {
  const id = req.swagger.params.id.value
  ...
  dbapi.getUserGroup(req.user.email, id).then(function(group) {
    ...
    let publishedGroup: Partial<GroupDocument> = apiutil.publishGroup(group)
...
function getGroups(req: ApiRequest, res: Response) {
  const fields = req.swagger.params.fields.value
  const owner = req.swagger.params.owner.value
  let getGenericGroups: typeof dbapi.getUserGroups
...
function createGroup(req: ApiRequest<GroupBody>, res: Response) {
...
function deleteGroups(req: ApiRequest<IdsBody>, res: Response) {
...
function deleteGroup(req: ApiRequest<IdsBody>, res: Response) {
  apiutil.redirectApiWrapper('id', deleteGroups, req, res)
}

function updateGroup(req: ApiRequest<GroupBody>, res: Response) {
  const id = req.swagger.params.id.value
  const lock: ApiLock = {}

  function updateUserGroup(
    group: GroupDocument
  , data: Parameters<typeof dbapi.updateUserGroup>[1]
  ) {
  ...
  lockutil.lockGroup(req, res, lock).then<Responded>(function(lockingSuccessed) {
    ...
    const repetitions =
      typeof req.body.repetitions === 'undefined' ?
        group.repetitions :
        req.body.repetitions
    const start = new Date(req.body.startTime || group.dates[0]!.start) as Instant
    const stop = new Date(req.body.stopTime || group.dates[0]!.stop) as Instant
    let state: string, isActive: boolean
    ...
    return checkSchedule(res, group, _class, group.owner.email, repetitions, group.privilege,
      ...
      if (start < (group.dates[0]!.start as Instant) ||
        stop > (group.dates[0]!.stop as Instant) ||
        repetitions > group.repetitions ||
        _class !== group.class) {
        return checkConflicts(id, group.devices, dates)
          .then<Responded>(function(conflicts) {
            if (!conflicts.length) {
...
function getGroupDevices(req: ApiRequest, res: Response) {
  const id = req.swagger.params.id.value
  ...
  dbapi.getUserGroup(req.user.email, id).then(function(group) {
    ...
    if (bookable) {
      ...
      dbapi.loadBookableDevices(req.user.groups.subscribed).then(function(devices) {
        Promise.map(devices, function(device: DeviceDocument) {
        ...
        .then(function(bookableSerials) {
          const deviceList: Partial<DeviceDocument>[] = []
