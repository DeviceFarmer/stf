...
import type {DeviceDocument, UserIdentity} from '../../types/stf.js'
...
export default db.ensureConnectivity(function(options: ProcessorOptions) {
  ...
  // Keys added by fingerprint alone, or before full keys were kept, get theirs the first time the
  // user connects with them. The key comes from the device, so it has to hash to the fingerprint
  // it is filed under before it is trusted.
  function storeAdbPublicKey(fingerprint: string, publicKey: string) {
    return adb.util.parsePublicKey(publicKey)
      .then(function(key) {
        if (key.fingerprint !== fingerprint) {
          throw new Error('Public key does not match its fingerprint')
        }
        return dbapi.setUserAdbPublicKey(fingerprint, publicKey)
      })
      .catch(function(err) {
        log.warn('Unable to store ADB public key "%s"', fingerprint, err.message)
      })
  }

  function sendAutoGroup(deviceChannel: string | Buffer, user: UserIdentity, identifier: string) {
    devDealer.send([
      deviceChannel
  ...
  function restoreDeviceOwner(device: DeviceDocument | null, deviceChannel: string) {
    ...
    return dbapi.loadUser(claim.email)
      .then<WriteResult | void>(function(user) {
        ...
        .then(function(user) {
          if (user) {
            ...
            if (message.publicKey) {
              storeAdbPublicKey(message.fingerprint, message.publicKey)
            }
          }
