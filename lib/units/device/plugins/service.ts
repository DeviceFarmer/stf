...
import ms from '../../../wire/messagestream.js'
...
import type {
  GetDisplayResponse
, KeyEventRequestFields
} from '../../../types/stfservice-wire.js'
...
import serviceSyrup from '../resources/service.js'
...
class MessageResolver {
  declare resolvers: Record<number, PendingCommand>
  declare await: (id: number, pending: PendingCommand) => Promise<Buffer>
  declare resolve: (id: number, value: Buffer) => Promise<Buffer>

  constructor() {
    this.resolvers = Object.create(null)

    this.await = function(this: MessageResolver, id, pending) {
      this.resolvers[id] = pending
      return pending.promise
    }

    this.resolve = function(this: MessageResolver, id, value) {
      var pending = this.resolvers[id]!
...
export default syrup.serial()
  ...
  , apk: StfServiceResource
    ...
    var plugin = new events.EventEmitter() as ServicePlugin

    var agent = {
      socket: null as LocalSocket | null
    , writer: null as Transform | null
    ...
    function callService(intent: string) {
    ...
    function prepareForServiceDeath(conn: LocalSocket) {
    ...
    function handleEnvelope(data: Buffer) {
    ...
    function openService() {
      ...
      return callService(util.format(
        ...
        .then(function(conn) {
          service.socket = conn
          service.reader = conn.pipe(new ms.DelimitedStream()) as Transform
          service.reader.on('data', handleEnvelope)
          service.writer = new ms.DelimitingStream() as Transform
          service.writer.pipe(conn)
          return prepareForServiceDeath(conn)
        })
    }

    function prepareForAgentDeath(conn: LocalSocket) {
      function endListener() {
        var startTime = Date.now()
    ...
    function openAgent() {
      ...
      return stopAgent()
        ...
        .then(function(conn) {
          agent.socket = conn
          agent.writer = new ms.DelimitingStream() as Transform
          agent.writer.pipe(conn)
          return prepareForAgentDeath(conn)
        })
    }

    function runAgentCommand(type: number, cmd: ServiceRequest) {
    ...
    function keyEvent(data: Partial<KeyEventRequestFields>) {
    ...
    function runServiceCommand(type: number, cmd: ServiceRequest) {
      var resolve_!: (value: Buffer) => void
      var promise = new Promise<Buffer>(function(resolve) {
        resolve_ = resolve
      })
      ...
      return messageResolver.await(id, {resolve: resolve_, promise: promise})
    }
    ...
    plugin.getDisplay = function(id) {
      return runServiceCommand(
        ...
        .timeout(10000)
        .then(function(data) {
          var response = apk.wire.GetDisplayResponse.decode(data) as
            NonNullableFields<GetDisplayResponse>
          if (response.success) {
            return {
              id: id
    ...
    plugin.getBrowsers = function() {
      return runServiceCommand(
        ...
        .then(function(data) {
          ...
          if (response.success) {
            delete (response as {success?: boolean}).success
            return response
          }
