...
var command = 'local [serial..]'

var describe = 'Start a complete local development environment.'
  ...
  return yargs
    ...
    .option('provider', {
      describe: 'An easily identifiable name for the UI and/or log output. ' +
        'Use --no-provider to run without a local provider.'
    , type: 'string'
    , default: os.hostname()
    })
    ...
    , coerce: function(val: string) {
        var size = val.split('x').map(Number)
        if (size.length !== 2 || !size.every(function(n) {
          return Number.isInteger(n) && n > 0
        })) {
          throw new Error('Invalid --vnc-initial-size "' + val + '", expected WIDTHxHEIGHT')
        }
        return size
      }
    })
...
var handler = function(argv: CommandArgv<typeof builder>) {
  ...
  var procutil = require('../../util/procutil.js').default
  ...
  function run() {
    var procs = [
      // app triproxy
      procutil.fork(path.resolve(import.meta.dirname, '..'), [
          'triproxy', 'app001'
    ...
    if ((argv.provider as string | false) !== false) {
      procs.push(procutil.fork(path.resolve(import.meta.dirname, '..'), [
          'provider'
        , '--name', argv.provider
        ...
        .concat(argv.cleanupFolder ? ['--cleanup-folder' as CliArg].concat(argv.cleanupFolder) : [])
  ...
  return procutil.fork(path.resolve(import.meta.dirname, '..'), ['migrate'])
    .done(run)
}
