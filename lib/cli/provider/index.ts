...
var command = 'provider [serial..]'

var describe = 'Start a provider unit.'
...
var handler = function(argv: CommandArgv<typeof builder, {serial: (string | number)[]}>) {
  ...
  return require('../../units/provider/index.js').default({
    name: argv.name
  , killTimeout: 10000
  ...
  , fork: function(device: Device, ports: number[]) {
      ...
      .concat(argv.connectSub.reduce<CliArg[]>(function(all, val) {
        return all.concat(['--connect-sub', val])
      }, []))
      .concat(argv.connectPush.reduce<CliArg[]>(function(all, val) {
