...
class Lifecycle {
  declare observers: Array<() => unknown>
  ...
  share<T>(name: string, emitter: T & ShareableEmitter, options?: ShareOptions): T {
    var opts = _.assign({
        end: true
      , error: true
      }
    ...
    if (opts.end) {
      emitter.on('end', function(this: Lifecycle) {
        if (!this.ending) {
    ...
    if (opts.error) {
      emitter.on('error', function(this: Lifecycle, err: Error) {
        if (!this.ending) {
