...
interface AdbKeyUtil {
  encodePublicKey(key: ExtendedPublicKey): string
}

var adbkeyutil: AdbKeyUtil = Object.create(null)
...
function toBigInt(forgeBigInteger: jsbn.BigInteger) {
  return BigInt('0x' + forgeBigInteger.toString(16))
}

function writeLittleEndian(buffer: Buffer, offset: number, value: bigint, length: number) {
  var rest = value
...
adbkeyutil.encodePublicKey = function(key: ExtendedPublicKey) {
