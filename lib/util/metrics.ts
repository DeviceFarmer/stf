...
type DeviceState =
  'absent' | 'present' | 'offline' | 'unauthorized' | 'preparing' | 'available' | 'busy'

interface MetricsDevice {
  present?: boolean
  ready?: boolean
  status?: number
  owner?: object | null
...
interface Metrics {
  ...
  deviceState(device: MetricsDevice): DeviceState
  aggregateDevices(devices: MetricsDevice[]): DeviceStats
  aggregateUsers(users: MetricsUser[]): UserStats
  aggregateGroups(groups: MetricsGroup[]): GroupStats
  update(devices: MetricsDevice[], users: MetricsUser[], groups: MetricsGroup[]): void
  register: client.Registry
  gauges: typeof gauges
}
...
function zeroFill(values: string[]) {
  return values.reduce(function(counts: Counts, value) {
    counts[value] = 0
    return counts
  }, Object.create(null))
}
...
function count(counts: Counts, value: string) {
...
function setLabeled(gauge: client.Gauge, label: string, counts: Counts) {
  gauge.reset()
...
metrics.deviceState = function(device: MetricsDevice) {
  if (!device.present) {
    return 'absent'
  }
...
metrics.aggregateDevices = function(devices: MetricsDevice[]) {
  const stats = {
...
metrics.aggregateUsers = function(users: MetricsUser[]) {
  const stats = {
    total: users.length
...
metrics.aggregateGroups = function(groups: MetricsGroup[]) {
  const stats = {
    total: groups.length
  , active: 0
