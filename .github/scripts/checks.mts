...
//
// The order is semantic: android-verdict.mts names the deepest layer that broke
// by walking it, and render-report.mts renders one table column per entry in the
...
//
// usage: node checks.mts seed <checks.json>
//
...
export type Checks = Record<string, string | undefined>
...
// failure how android-verdict.mts words a failure of this layer.
...
var RANK: Record<string, number> = {fail: 3, pass: 2, skip: 1}

function merge(current: string | undefined, incoming: string) {
  return RANK[incoming]! > (RANK[String(current)] || 0) ? incoming : current
}
