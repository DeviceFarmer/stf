...
//
// usage: node render-report.mts <verdicts-dir> <output.md>
//
...
interface MatrixSpec {
  include?: {android: unknown, api: unknown}[]
}

var ICON: Record<string, string> = {
...
var CHECK_ICON: Record<string, string> = {
  pass: ':white_check_mark:'
, fail: ':x:'
, warn: ':warning:'
...
function expectedAndroid() {
  ...
  if (file && fs.existsSync(file)) {
    try {
      var spec = JSON.parse(fs.readFileSync(file, 'utf8')) as MatrixSpec
      return (spec.include || []).map(function(leg) {
        return {android: String(leg.android), api: String(leg.api)}
      })
    }
    catch (e) {
      console.error('cannot read %s: %s', file, (e as Error).message)
    }
  }
...
function addMissing(verdicts: Verdict[]) {
...
function collect(dir: string) {
  ...
  fs.readdirSync(dir).forEach(function(entry) {
    ...
    if (stat.isDirectory()) {
      ...
      try {
        var parsed = JSON.parse(fs.readFileSync(full, 'utf8')) as Verdict | null
        if (parsed && parsed.id) {
          verdicts.push(parsed)
        }
      }
      catch (e) {
        console.error('skipping unparsable verdict %s: %s', full, (e as Error).message)
      }
    }
  })
  return verdicts
...
function checkIcon(status: string | undefined) {
  if (!status) {
...
function tally(list: Verdict[]) {
  var counts: {fail: number, [status: string]: number} = {pass: 0, fail: 0, skip: 0, cancel: 0}
  list.forEach(function(v) {
    if (typeof counts[v.status] === 'undefined') {
      counts[v.status] = 0
    }
    counts[v.status]!++
  })
