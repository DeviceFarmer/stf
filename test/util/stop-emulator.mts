...
import stopEmulator from '../../.github/scripts/stop-emulator.mts'

describe('CI emulator cleanup', function() {
  var sandbox: sinon.SinonSandbox, ps: sinon.SinonStub, adb: sinon.SinonStub
    , kill: sinon.SinonStub, running: string[]
  ...
  beforeEach(function() {
    ...
    adb = (sandbox.stub(childProcess, 'spawnSync') as sinon.SinonStub).returns({status: 0})
    kill = (sandbox.stub(process, 'kill') as sinon.SinonStub).callsFake(function(pid: number) {
  ...
  it('tolerates the emulator exiting between the process lookup and signal', function() {
    kill.callsFake(function() {
      running.shift()
      var error: NodeJS.ErrnoException = new Error('No such process')
      error.code = 'ESRCH'
      throw error
    })
    stopEmulator('5554')
...
describe('CI emulator cleanup wrapper', function() {
  ...
  var wrapper = path.resolve(import.meta.dirname, '../../.github/scripts/with-emulator-cleanup.sh')
  ...
  beforeEach(function() {
    ...
    fs.writeFileSync(path.join(directory, '.github/scripts/stop-emulator.mts'),
