...
import ldaputil from '../../lib/util/ldaputil.js'

interface LdapRequest {
  dn: object
  credentials: string
  filter: {matches(entry: object): boolean}
}
...
type LdapHandler = (req: LdapRequest, res: LdapResponse, next: (err?: Error) => void) => void

interface LdapServer {
  server: net.Server
  bind(base: string, handler: LdapHandler): void
  search(base: string, handler: LdapHandler): void
...
type LdapOptions = Parameters<typeof ldaputil.login>[0] & {username: {field: string}}

const require = createRequire(import.meta.url)
var ldap: Ldapjs = require('ldapjs')
...
describe('ldaputil', function() {
  describe('login', function() {
    ...
    var server: LdapServer, options: LdapOptions, lastFilter: string | null

    function normalize(dn: object) {
    ...
    beforeEach(function(done) {
      ...
      server.listen(0, '127.0.0.1', function() {
        options = {
          url: 'ldap://127.0.0.1:' + (server.server.address() as net.AddressInfo).port
        , timeout: 5000
