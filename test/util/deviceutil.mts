...
import deviceutil from '../../lib/util/deviceutil.js'

describe('deviceutil', function() {
  describe('isAddable', function() {
    function free(extra?: object): deviceutil.Holdable {
  ...
  describe('isClaimLive', function() {
    ...
    function device(extra?: object): deviceutil.Claimable {
  ...
  describe('isRestorable', function() {
    ...
    function user(groups: string[]) {
      return {
        email: 'user@example.com'
      , name: 'User'
      , group: 'channel1'
      , groups: {subscribed: groups}
      } as unknown as deviceutil.Viewer
    }
    ...
    it('should not restore when the user has no subscriptions recorded', function() {
      expect(deviceutil.isRestorable(device(), {email: 'user@example.com'} as deviceutil.Viewer
      , NOW)).to.be.false
    })
  })
})
