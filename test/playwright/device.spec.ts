...
//
// Test titles carry a [check:<key>] tag; .github/scripts/playwright-checks.mts
...
test.describe('STF against a real device', function() {
  ...
  test('[check:screen_stream] minicap frames reach the browser canvas',
    async function() {
      ...
      await expect.poll(async function() {
        return canvas.evaluate(function(el: HTMLCanvasElement) {
      ...
      const before = await canvas.evaluate(function(el: HTMLCanvasElement) {
        return el.toDataURL()
      })
      ...
      await expect.poll(async function() {
        const now = await canvas.evaluate(function(el: HTMLCanvasElement) {
          return el.toDataURL()
        })
        return now !== before
  ...
  test('[check:touch_roundtrip] a tap in the browser becomes a touch on the device',
    async function({}, testInfo) {
      ...
      let positions: h.TouchPositions = {device: null, x: [], y: []}
      let fractions: h.Fractions | null = null
