...
export interface Recorder {
  log(): string
  stop(localPath: string): Promise<string | null>
}

export interface AxisMax {
  x: number
  y: number
}

export type AxisRanges = Record<string, AxisMax>
...
async function waitForDeviceTile(page: Page, timeout?: number) {
...
function captureInputEvents(serial?: string, seconds?: number): Capture {
...
async function attachCapture(testInfo: TestInfo, name: string, capture: Capture, header?: string) {
...
function describeGesture(positions: TouchPositions, fractions: Fractions | null, aimed: string) {
  const round = function(v: number) {
  ...
  ;(['x', 'y'] as const).forEach(function(axis) {
...
function adb(args: string[], rest: string[]) {
  return new Promise<number | null>(function(resolve) {
...
async function gone(args: string[], timeout: number) {
...
function closed(child: ChildProcess, timeout: number) {
...
// never fail a test.
function recordDeviceScreen(serial?: string, seconds?: number): Recorder {
  const args = serial ? ['-s', serial] : []
  ...
  , stop: async function(localPath: string) {
...
async function saveRecording(recorder: Recorder, testInfo: TestInfo, name: string) {
...
function hasTouchEvents(log: string) {
...
function parseAxisRanges(text: string) {
  ...
  text.split(/\r?\n/).forEach(function(line) {
    ...
    let key: 'x' | 'y' | null = null
    if (code === '0035' || code === '0000') {
      key = 'x'
    }
    else if (code === '0036' || code === '0001') {
      key = 'y'
    }
    if (!key) {
      return
    }
    ranges[device] = ranges[device] || ({} as AxisMax)
    // A device that reports both ABS_X and ABS_MT_POSITION_X keeps the MT one,
    // because that is the axis minitouch drives.
    if (typeof ranges[device]![key] === 'undefined' || code === '0035' || code === '0036') {
...
function touchAxisRanges(serial?: string) {
  const args = serial ? ['-s', serial] : []
  return new Promise<AxisRanges>(function(resolve) {
...
function parseTouchPositions(log: string): TouchPositions {
  const byDevice: Record<string, {x: number[], y: number[]}> = {}
...
function toFractions(positions: TouchPositions, ranges: AxisRanges): Fractions | null {
  if (!positions.device) {
    return null
  }
...
function isPermissionProblem(log: string) {
...
function isCaptureBroken(log: string) {
...
async function tapDeviceScreen(page: Page, fx?: number, fy?: number) {
  ...
  const x = box.x + (typeof fx === 'undefined' ? 0.5 : fx) * box.width
  const y = box.y + (typeof fy === 'undefined' ? 0.5 : fy) * box.height
...
async function swipeDeviceScreen(page: Page, from: [number, number], to: [number, number]) {
  const box = await page.locator(SEL.canvas).boundingBox()
  if (!box) {
    throw new Error('device screen canvas has no box, screen is not rendered')
  }
  const point = function(f: [number, number]): [number, number] {
