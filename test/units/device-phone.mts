...
import phoneSyrup from '../../lib/units/device/plugins/util/phone.js'
...
interface FetchOptions {
  ...
  shell?: (command: string) => PromiseLike<Readable>
}

function agentOutput(text: string | null) {
...
function fetch(options: FetchOptions) {
  var adb = {
    shell: sinon.stub().callsFake(function(serial: string, command: string) {
...
function runShell(uid: number, command: string) {
  var directory = fs.mkdtempSync(path.join(os.tmpdir(), 'stf-phone-'))
  var scripts: Record<string, string> = {
    id: 'printf "%s\\n" "$STF_TEST_UID"\n'
  , su: '[ "$1" = shell ] || exit 1\n' +
...
describe('device phone info', function() {
  ...
  ;[{} as Properties, {'gsm.sim.state': ''}].forEach(function(deviceProperties) {
