...
import protobuf from '../../lib/wire/protobuf.js'
import pathutil from '../../lib/util/pathutil.js'
import type {LogcatFilter} from '../../lib/types/wire.js'

type Wire = typeof wire
type DecodableName = {
  [K in keyof Wire]: Wire[K] extends {decode(buffer: Buffer): object} ? K : never
}[keyof Wire]
...
interface ServiceMessageClass {
  new(...values: unknown[]): ServiceMessage
  decode(buffer: Uint8Array): {[field: string]: unknown}
}

interface ServiceWire {
  RingerMode: Record<string, number>
  ClipboardType: Record<string, number>
  KeyEvent: Record<string, number>
  MessageType: Record<string, number>
  Envelope: ServiceMessageClass
  DoTypeRequest: ServiceMessageClass
  GetDisplayRequest: ServiceMessageClass
  KeyEventRequest: ServiceMessageClass
  RotationEvent: ServiceMessageClass
}

type ServiceRoot = {jp: {co: {cyberagent: {stf: {proto: ServiceWire}}}}}

type LooseConstructor = new(...values: unknown[]) => object

var pbjs5MessageType: Record<string, number> = {
  CopyMessage: 33
...
function fields(message: object) {
...
describe('wire protobuf', function() {
  ...
  describe('message validation', function() {
    it('should reject a field value of the wrong type', function() {
      expect(function() {
        return new wire.KeyDownMessage(123 as unknown as string).encodeNB()
  ...
  describe('wire format', function() {
    ...
    it('should decode an unset repeated field as a mutable empty array', function() {
      ...
      decoded.filters.push('anything' as unknown as LogcatFilter)
  ...
  describe('STFService wire', function() {
    ...
    var svcwire = (protobuf.build(svc) as unknown as ServiceRoot).jp.co.cyberagent.stf.proto
